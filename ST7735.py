# TFT class edited by Bonjir
# font & char_dict & textout() & char() added
# showbmp() added
...
def TFTColor(r,g,b) :
    return (((0xFF - b) & 0xF8) << 8) | (((0xFF - g) & 0xFC) << 3) | ((0xFF - r) >> 3)

ScreenSize = (80, 160)
offset_array = [[26, 1], [1, 26], [26, 1], [1, 26]]
...
class TFT(object) :
    ...
    def __init__( self, spi, aDC, aReset, aCS) :
        ...
        self._offset = bytearray(offset_array[0])
        ...
        self.fontw = self.DEFAULT_FONTW
        self.fonth = self.DEFAULT_FONTH
        self.char_dict = self.DEFAULT_CHAR_DICT
        self.font_file = open(self.DEFAULT_FONT_PATH, "rb")
        self.font_size = (1, 1)
    ...
    def rotation( self, aRot ) :
        ...
        if (0 <= aRot < 4):
            ...
            if (rotchange & 1):
                ...
                self._offset = bytearray(offset_array[self.rotate])
            self._setMADCTL()
...
#                                                         0x30, 0x39, 0x3f, 0x00, 0x07, 0x03, 0x10])
    ...
    def setfont( self, char_dict, font_path, aSize = (1, 1) ) :
        self.char_dict = char_dict
        
        self.font_file.close()
        self.font_file = open(font_path, "rb")
        self.font_size = aSize
    
    def setfontsize( self, aSize : tuple ):
        self.font_size = aSize
    
    def char( self, aPos, aChar, aColor, aSize = (1, 1) ):
        aSize = (aSize[0] * self.font_size[0], aSize[1] * self.font_size[1])
        if aChar in self.char_dict :
            char_bytes = []
            self.font_file.seek(self.char_dict[aChar], 0)
            for col in range(0, self.fontw):
                char_bytes.append(int.from_bytes(self.font_file.read(1), "little"))
            for col in range(0, self.fontw):
                for pixel in range(0, self.fonth):
                    b = (char_bytes[col] & (1 << pixel))
                    if b:
                        if aSize == 1:
                            self.pixel((aPos[0] + col, aPos[1] + pixel), aColor)
                        else:
                            self.fillrect((aPos[0] + col * aSize[0], aPos[1] + pixel * aSize[1]), (aSize[0], aSize[1]), aColor )
        else :
            self.fillrect((aPos[0] + 1 * aSize[0], aPos[1]), ((self.fontw - 1) * aSize[0], (self.fonth - 1) * aSize[1]), aColor)
            
    def textout( self, aPos, aStr, aColor, auto_return = True, aSize = (1, 1) ):
        aSize_scale = aSize
        aSize = (aSize[0] * self.font_size[0], aSize[1] * self.font_size[1])
        aPos0 = aPos
        for i in range(0, len(aStr)):
            if auto_return and aPos[0] + (i + 1) * self.fontw * aSize[0] >= ScreenSize[1]:
                aPos = (aPos0[0] - i * self.fontw * aSize[0], aPos[1] + self.fonth * aSize[1])
            if  aStr[i] == '\n':
                aPos = (aPos0[0] - (i + 1) * self.fontw * aSize[0], aPos[1] + self.fonth * aSize[1])
                continue
            self.char((aPos[0] + i * self.fontw * aSize[0], aPos[1]), aStr[i], aColor, aSize_scale)
    
    def showbmp( self, aPos, bmppath : str ):
        f=open(bmppath, 'rb')
        if f.read(2) == b'BM':  # header
            ...
            if int.from_bytes(f.read(2), 'little') == 1: # color planes must be 1
                depth = int.from_bytes(f.read(2), 'little') # number of pxls
                compress_method = int.from_bytes(f.read(4), 'little')
                if (depth == 24 or depth == 32) and compress_method == 0: # compress method == uncompressed
                    pxl_size = 3 if depth == 24 else 4
#                     print("Image size:", width, "x", height)
                    rowsize = (width * pxl_size + pxl_size) & ~pxl_size
                    if height < 0:
                        height = -height
                        flip = False
                    else:
                        flip = True
                    w, h = width, height
                    self._setwindowloc((0 + aPos[0],0 + aPos[1]),(w - 1 + aPos[0],h - 1 + aPos[1]))
                    for row in range(h):
                        if flip:
...
class TFT_Font (TFT) :
    ...
    'g' : 426, 'h' : 432, 'i' : 438, 'j' : 444, 'k' : 450, 'l' : 456, 'm' : 462, 'n' : 468, 'o' : 474,
    'p' : 480, 'q' : 486, 'r' : 492, 's' : 498, 't' : 504, 'u' : 510, 'v' : 516, 'w' : 522, 'x' : 528,
    'y' : 534, 'z' : 540, 'horiz line' : 546
    }
    FONT_PATH_6X8 = "codetab.bin"

    def __init__( self, spi, aDC, aReset, aCS) :
        super(TFT_Font, self).__init__( spi, aDC, aReset, aCS )
        self.char_dict = {}
    
    def setfont( self, char_dict, font_path ) :
        self.char_dict = char_dict
        self.font_file = open(font_path, "rb")
    
    def char( self, aPos, aChar, aColor ):
        if aChar in self.char_dict :
            char_bytes = []
            self.font_file.seek(self.char_dict[aChar], 0)
            for col in range(0, self.FONTW):
            ...
            self.fillRect((aPos[0] + 1, aPos[1]), (self.FONTW - 1, self.FONTH - 1), aColor)
            
    def textout( self, aPos, aStr, aColor ):
        for i in range(0, len(aStr)):
            self.char((aPos[0] + i * self.FONTW, aPos[1]), aStr[i], aColor)
"""